...
|---|---|---|---|---|
| B5.1 | App-support-root test seam: honor `PATCHCORD_APPSUPPORT_ROOT` env override in `MacDiscordPlatform` (`AppSupportRoot`/`AppSupportDirForBranch`) so harnesses can point Layer-B resolution at a `/tmp` fixture without mutating the real Discord tree (mirrors `VENCORD_USER_DATA_DIR`). Default unchanged = `~/Library/Application Support`. | `src/PatchCord.Mac/MacDiscordPlatform.cs` | `dotnet build PatchCord.sln -p:EnableWindowsTargeting=true` (3/3 green) + `dotnet build src/PatchCord.Core/PatchCord.Core.csproj` (flag-free) | DONE (69fad36) |
| B5.2 | BD asar presence + Core inject/restore (Layer B, FDA-free): assert `BetterDiscordAsarPath` resolves to `~/Library/Application Support/BetterDiscord/data/betterdiscord.asar` (matches BD installer `install.js`); with the asar present, drive `BetterDiscordEngine.Inject(coreAppDir, asar)` on the real wrapped `app-0.0.395` core, read `index.js` back, assert it `== InjectContent` (contains `betterdiscord.asar`); then `Restore`, assert byte-exact vanilla `module.exports = require('./core.asar');\n` (41 bytes). | `src/PatchCord.Mac/Program.cs` (new `--mac-b5-bdtest`) | `dotnet run --project src/PatchCord.Mac -- --mac-b5-bdtest` → all PASS, exit 0 | DONE (f0b6291) |
| B5.3 | BD (BandagedBD) live load (the one unavoidable GUI step): place `betterdiscord.asar` (`curl https://betterdiscord.app/Download/betterdiscord.asar`), set an install's `ClientMod=betterdiscord`, let the monitor inject + restart, then **visually** confirm BetterDiscord/BandagedBD loaded (BD section in Discord settings). NOTE: uninstall/"toggle off" is NOT a product feature — PatchCord installs the mod into Discord's launcher and keeps it re-patched after updates; it does not uninstall packages from the UI (so no "removed after toggle-off" confirm). | (none — runtime confirm) | Manual: launch `PatchCord.app` → set BetterDiscord → BandagedBD appears in Discord settings | DONE (manual, 2026-06-18) |
| B5.4 | OpenAsar FDA-free confirm (on `/tmp` copy): (a) download + 12h cache TTL — backdate cache mtime → re-download, freshen → cache hit; (b) positive detection — `Install` OpenAsar into a `/tmp` Resources copy, assert `IsInstalled==true`; (c) layering order — install OpenAsar then `PatchEngine.Patch` a stub on top then `Unpatch`, assert OpenAsar layer survives + still detected. No real bundle write. | `src/PatchCord.Mac/Program.cs` (new `--mac-b5-openasar`), may extend `OpenAsarEngine.TestFetchAndDetect` (`src/PatchCord.Core/OpenAsarEngine.cs`) | `dotnet run --project src/PatchCord.Mac -- --mac-b5-openasar` → all PASS, exit 0 | DONE (f0b6291) |
| B5.5 | Monitor BD re-inject after a new `app-<ver>` (re-patch loop, FDA-free via B5.1 seam): build a `/tmp` fixture app-support root (`app-0.0.395` + higher `app-0.0.999`, each vanilla wrapped `index.js`), `ClientMod=betterdiscord`. (a) Run `MonitorService.RunOnce` with `MonitoringEnabled=false` (so the loop reconciles over the fixture WITHOUT stopping/patching/restarting the real Discord — the full managed-patch path needs a real running Discord, not headlessly reachable per design §16.3); assert it targets the higher core (`LastStates[inst].AppDir` = `app-0.0.999`) and does not patch (`Recorded=false`). (b) Assert the direct re-inject invariant: `ResolveCoreAppDir` returns the higher dir and `BetterDiscordEngine.Inject` lands `betterdiscord.asar` in its `index.js` while the old `app-0.0.395` stays vanilla. | `src/PatchCord.Mac/Program.cs` (new `--mac-b5-repatch`) | `dotnet run --project src/PatchCord.Mac -- --mac-b5-repatch` → PASS, exit 0 | DONE (f0b6291) |
| B5.6 | Version-ordering unit (no Discord): over a `/tmp` fixture with `app-0.0.9`, `app-0.0.10`, `app-weird`, assert `GetLatestCoreAppDir` picks `0.0.10` (numeric, not lexical) and ignores the non-`X.Y.Z` name. | `src/PatchCord.Mac/Program.cs` (fold into `--mac-b5-repatch` or `--mac-selftest`) | `dotnet run --project src/PatchCord.Mac -- --mac-b5-repatch` → version-order checkpoint PASS | DONE (f0b6291) |
...
- 2026-06-18 — **B5.3 CONFIRMED + product-model correction + UI defect found.** Live run of the packaged
  ...
  (user):** PatchCord only *installs* a client mod into Discord's launcher and keeps it re-patched after
  updates — it does NOT uninstall packages from the UI; "toggle off → removed" is not a feature, so that
  confirm was dropped from B5.3. (2) **UI defect:** the Avalonia UI rendered broken on first load and only
  showed the installed state after a manual **Refresh** — tracked as a follow-up (not a Core/patch-engine
  fault; the patch flow itself worked). The patch mechanism and re-patch loop are sound; the bug is in the
  Mac shell's view/state binding.
- 2026-06-18 — **B5 IMPLEMENTED (Layer-B confirm + OpenAsar/monitor hardening), branch feat/macos-port.**
  8 of 9 checkpoints DONE and green; B5.3 (the one unavoidable GUI step — visually confirm BetterDiscord
  loaded) is PENDING the user's manual pass. Commits: bbed2fb (B5 plan), 69fad36 (B5.1 app-support seam +
  B5.7 ShipIt Discord-identity fix + B5.8 restart-by-path fix), f0b6291 (B5.2/B5.4/B5.5/B5.6 headless
  harnesses + OpenAsar cache-timestamp helper). Verified on this Mac: sln 3/3 green, Core builds flag-free,
  `--mac-b5-shipit` 4/4 (foreign-Squirrel false-positive regression guard: stub confirmed seen by
  GetProcessesByName then rejected; Discord-identity stub accepted), `--mac-b5-bdtest` 3/3 (BD asar present;
  inject == InjectContent; restore byte-exact 41-byte vanilla; real index.js confirmed restored),
  `--mac-b5-openasar` 3/3 (download 41 KB + 12h cache hit/re-download + positive detection + layering
  survives a Layer-A round-trip, all on a /tmp copy), `--mac-b5-repatch` 2/2 (MonitorService.RunOnce
  targets the new app-0.0.999 core without patching while monitoring off + BD re-inject invariant +
  numeric version ordering), `--mac-selftest` 8/8 (no regression). Layer-A live-confirm + OpenAsar real
  bundle install/uninstall remain PARKED behind the FDA grant.
